...
class DwcTrainer:
    ...
    def train(self, mode=0, batch_size=1, sample_training_length=1, feedback_timer=10, stop_condition=1e-2,
        ...
        with torch.no_grad():
            if mode == 0:
                target = torch.rand(batch_size, *LATTICE_SMALL, GAUGE_DOF, NON_GAUGE_DOF, dtype=torch.complex64)
                current_B = d_wc(target)
            elif mode == 1:
                current_B = torch.rand(NUM_OF_LATTICES, *LATTICE_SMALL, GAUGE_DOF, NON_GAUGE_DOF, dtype=torch.complex64)
        ...
        try:
            ...
            while True:
                ...
                if counter % sample_training_length == 0:
                    with torch.no_grad():
                        if mode == 0:
                            target = torch.rand(batch_size, *LATTICE_SMALL, GAUGE_DOF, NON_GAUGE_DOF,
                                                dtype=torch.complex64)
                            current_B = d_wc(target)
                        elif mode == 1:
                            current_B = torch.rand(NUM_OF_LATTICES, *LATTICE_SMALL, GAUGE_DOF, NON_GAUGE_DOF,
                                                   dtype=torch.complex64)
                            target = gmres(d_wc, current_B, best_preconditioner, best_preconditioner(current_B))[0]
