...
def arnoldi(V, v_temp, i_index):
    ...
    v_out = w_temp / h_out  # V[:, i + 1]
    return h_out, v_out


def gmres(operator: nn.Module, b: torch.Tensor, max_iter=100, tol=1e-3, print_res=False):
    time_before = time()
    y_i = torch.zeros(0)
    ...
    for i_index in range(max_iter):
        v_temp = operator(V[:, i_index].reshape(b_shape)).view(-1)
        H[i_index + 1, i_index], V[:, i_index + 1] = arnoldi(V, v_temp, i_index)
...
def gmres_precon(operator: nn.Module, b: torch.Tensor, preconditioner: nn.Module, max_iter=100, tol=1e-3,
                 print_res=False):
    time_before = time()
    y_i = torch.zeros(0)
    ...
    for i_index in range(max_iter):
        ...
        v_temp = operator(Z[:, i_index].reshape(b_shape)).view(-1)
        H[i_index + 1, i_index], V[:, i_index + 1] = arnoldi(V, v_temp, i_index)
...
def gmres_train(operator: nn.Module, b: torch.Tensor, preconditioner: nn.Module, pure_gmres_iter: int, tol=1e-3):
