...
def covariance_test(module: nn.Module):
    ...
    test_gauge = torch.rand(DEFAULT_BATCH_SIZE, *LATTICE_SMALL, GAUGE_DOF, GAUGE_DOF, dtype=torch.complex64)
    test_gauge = torch.qr(test_gauge)[0]  # Make rand complex 3x3 Orthogonal with QR deconstruction -> unitary matrix

    output_1 = gauge_tra(module(test_field), test_gauge)  # Transform output

    module.gauge_tra(test_field)  # Transform module
    output_2 = module(gauge_tra(test_field, test_gauge))  # Transform input

    result = torch.mean(torch.abs(output_1 - output_2))
