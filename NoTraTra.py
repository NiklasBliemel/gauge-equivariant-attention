...
class ReducedNonGaugeLinear(nn.Module):
    def __init__(self, non_gauge_dof, head_dim, lattice):
        ...
        blocks = torch.randn(self.lattice_dof, non_gauge_dof, head_dim // 4, dtype=torch.complex128)
        # Create the diagonal block matrix
        block_matrix = torch.zeros(non_gauge_dof * self.lattice_dof, head_dim, dtype=torch.complex128)
        block_matrix[:non_gauge_dof, :head_dim // 4] = blocks[0]
        block_matrix[non_gauge_dof:2 * non_gauge_dof, head_dim // 4:head_dim // 2] = blocks[1]
        block_matrix[2 * non_gauge_dof:3 * non_gauge_dof, head_dim // 2:3 * head_dim // 4] = blocks[2]
        block_matrix[3 * non_gauge_dof:4 * non_gauge_dof, 3 * head_dim // 4:head_dim] = blocks[3]
...
class TraceActivation(nn.Module):
    ...
    def forward(self, attention):
        ...
        print(torch.sum(effect.reshape(1, 1, 4, 4, 4, 8, -1)[0, 0, 0, 1, 2, 3]))
        print(effect.reshape(1, 1, 4, 4, 4, 8, 4, 4, 4, 8)[0, 0, 0, 1, 2, 3, 0, :3, :, :])
...
class SelfAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        out = out.reshape(*values_shape[:-1], -1)
        return out
